...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { token, user } = await api.signup(trimmedEmail, password);
      setAuthToken(token);
      login(token, user);
    ...
    <div className="min-h-screen bg-[#0a0a0c] flex items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-xl bg-slate-900/80 border border-slate-800 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="signup-email" className="block text-xs font-medium text-slate-500 mb-1.5">Email</label>
            <input
              id="signup-email"
              type="email"
              value={email}
          ...
          <div>
            <label htmlFor="signup-password" className="block text-xs font-medium text-slate-500 mb-1.5">Password</label>
            <input
              id="signup-password"
              type="password"
              value={password}
